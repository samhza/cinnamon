...
def outline(img: pyvips.Image, radius: int) -> pyvips.Image:
    img = img.embed(radius, radius, img.width + radius * 2, img.height + radius * 2, extend="copy")
...
def meme(width: int, height: int, top: str, bottom: str) -> pyvips.Image:
    ...
    def meme_text(text: str) -> pyvips.Image | None:
        if text == "":
            return None
        text = pyvips.Image.text(text, font=f"Impact Bold {width/9}", width=width*.95, height=height*.95/3, align="centre", rgba=True, dpi=72)
