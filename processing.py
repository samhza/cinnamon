...
def write_overlay(
    func: typing.Callable, width: int, height: int, *args, **kwargs
) -> str:
...
class Processing:
    ...
    async def dimensions(
        self, filepath: str, media_type: str
    ...
    async def ffmpeg_overlay(
        self, input: File, func: typing.Callable, *args, **kwargs
    ) -> str:
        overlay_file = None
    ...
    async def stack(self, orientation, file1, file2) -> str:
        ...
        input1 = (
            ffmpeg.input(file1.name)
            .video.filter("setpts", "PTS-STARTPTS")
            .filter("format", "yuv420p")
        )
        input2 = (
            ffmpeg.input(file2.name)
            .video.filter("setpts", "PTS-STARTPTS")
            .filter("format", "yuv420p")
        )
